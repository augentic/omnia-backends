...
When a completion runs, `omnia-cursor` reads the grant's `url`, merges the entry into `<workspace>/.cursor/mcp.json` for the spawn, and passes `--approve-mcps`. `cursor-agent` has no `--mcp-config` flag; it discovers servers from the git toplevel of `--workspace` (or `~/.cursor/mcp.json`), so the backend `git init`s the workspace when it is not already a root.